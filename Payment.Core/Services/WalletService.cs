...
namespace Payment.Core.Services
{
    public class WalletService : IWalletService
    {
        ...
        public async Task<ResponseDto<CreateWalletResponse>> CreateWalletAsync(CreateWalletRequest walletRequestDto)
        {
            ...
            // Start a new transaction scope with async support
            using var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled);

            try
            {
                // Register user as Paystack customer
                var customerResponse = await _payStackPaymentHandler.CreateCustomer(walletRequestDto);
                if (!customerResponse.status)
                {
                    _logger.Information($"Could not create Paystack customer account for user {walletRequestDto.UserId} : {customerResponse.message}");
                    return ResponseDto<CreateWalletResponse>.Fail($"Could not create Paystack customer account for user: {customerResponse.message}", (int)HttpStatusCode.BadRequest);
                }

                // Create wallet object and map data from the request DTO
                wallet = _mapper.Map<Wallet>(walletRequestDto);
                wallet.PaystackCustomerCode = customerResponse.data.customer_code;
                wallet.Pin = BCrypt.Net.BCrypt.HashPassword(walletRequestDto.Pin);  // Hash the pin before saving
                wallet.Code = ShortId.Generate(new GenerationOptions(length: 8));    // Generate a short wallet code

                // Add wallet to the database
                await _unitOfWork.Wallets.AddAsync(wallet);

                // Create the virtual account request object
                ...
                // Error handling for virtual account creation
                try
                {
                ...
                {
                    _logger.Error($"Argument error creating virtual account: {argEx.Message}, StackTrace: {argEx.StackTrace}");
                    return ResponseDto<CreateWalletResponse>.Fail($"Invalid data provided for virtual account: {argEx.Message}", (int)HttpStatusCode.BadRequest);
                }
                catch (HttpRequestException httpEx) // Handling service unavailability or connection errors
                {
                    _logger.Error($"HTTP request error creating virtual account: {httpEx.Message}, StackTrace: {httpEx.StackTrace}");
                    return ResponseDto<CreateWalletResponse>.Fail($"Virtual account creation service is unavailable: {httpEx.Message}", (int)HttpStatusCode.ServiceUnavailable);
                }
                catch (Exception ex)  // Generic exception handling
                {
                    _logger.Error($"Unexpected error creating virtual account: {ex.Message}, StackTrace: {ex.StackTrace}");
                    return ResponseDto<CreateWalletResponse>.Fail($"Unexpected error creating virtual account: {ex.Message}", (int)HttpStatusCode.InternalServerError);
                }
            ...
            {
                _logger.Error($"Could not create wallet: {ex.Message}, StackTrace: {ex.StackTrace}");
                scope.Dispose(); // Disposing of the transaction scope in case of failure
                return ResponseDto<CreateWalletResponse>.Fail($"Could not create wallet: {ex.Message}", (int)HttpStatusCode.BadRequest);
            }

            // Map the wallet data to a response DTO
